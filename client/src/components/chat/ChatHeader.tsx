...
interface Props {
  ...
  onBack: () => void;
  onOpenInfo: () => void;   // opens ContactInfoPanel
  isInfoOpen: boolean;
}
...
export default function ChatHeader({ conversation, onBack, onOpenInfo, isInfoOpen }: Props) {
  ...
  return (
    <div className="flex items-center gap-2 px-3 sm:px-4 py-3 bg-white dark:bg-neutral-900 border-b border-neutral-100 dark:border-neutral-800 shrink-0">
      ...
      {/* Clickable avatar + name → opens info panel */}
      <button
        onClick={onOpenInfo}
        className="flex items-center gap-3 flex-1 min-w-0 text-left rounded-xl px-1 py-0.5 hover:bg-neutral-50 dark:hover:bg-neutral-800/60 transition-colors"
      >
        {/* Avatar */}
          ...
          {isGroup ? (
            ...
            <>
              <Avatar className="w-10 h-10 rounded-2xl shadow-sm">
                <AvatarImage src={pic} alt={other?.username} className="rounded-2xl object-cover" />
